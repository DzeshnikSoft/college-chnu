...
$dotnetSolutionPath = $codePath + '\services'
$reactAppPath = $codePath + "\web"
...
Write-Host "*************************🔥 Starting College.API 🔥*************************"

$dotnetProcess = Start-Process -FilePath dotnet -ArgumentList "watch run --verbosity m --project .\services\College.API\College.API.csproj" -PassThru
